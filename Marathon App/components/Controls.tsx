...
interface ControlsProps {
  ...
  onSetGoals: () => void;
  onDownloadCalendar: () => void; // <-- ADD THIS NEW PROP
}
...
const Controls: React.FC<ControlsProps> = ({
  ...
  isSuggestingPlan,
  onSetGoals,
  onDownloadCalendar, // <-- DESTRUCTURE THE NEW PROP
}) => {
  ...
  // Adjusted for consistency with other action buttons
  const actionButtonBaseStyles = "inline-flex items-center justify-center px-5 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 transition-all duration-150 ease-in-out";
  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4 p-4 bg-slate-800/50 rounded-xl shadow-lg border border-slate-700">
      ...
      <div className="flex flex-wrap gap-2 sm:space-x-2"> {/* Added flex-wrap and gap-2 for better responsiveness */}
        <button
          onClick={onSuggestPlan}
          disabled={isSuggestingPlan}
          className={`${actionButtonBaseStyles} bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 focus:ring-purple-500 disabled:opacity-50 disabled:cursor-not-allowed`}
        >
          {isSuggestingPlan ? (
            <>
        ...
        <button
          ...
          className={`${actionButtonBaseStyles} bg-gradient-to-r from-emerald-500 to-green-500 hover:from-emerald-600 hover:to-green-600 focus:ring-emerald-500`}
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-2">
        ...
        <button
          ...
          className={`${actionButtonBaseStyles} bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500`}
          title="Download upcoming activities as an ICS calendar file"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-2">
